...
def generate_vocab_info(word):
    ...
    fallback_data = {
        ...
        "advanced_sentence": f"Students should master the usage of {w_clean} in context.",
...
if main_menu == "✨ 智慧單字新增":
    ...
    if df_vocab.empty:
        ...
        with st.expander("📋 點擊收合/展開：單字總表與快速編輯區", expanded=True):
            ...
            # 💡 利用 Pandas 原生 Index，將 Index 命名為「編號」
            display_df.index = range(1, len(display_df) + 1)
            display_df.index.name = '編號'
            
            display_columns = ['word', 'phonetic', 'part_of_speech', 'definition']
            
            # 使用原生的 st.dataframe 並保留 index，Streamlit 會自動將 index 顯示得窄小且完美對齊
            st.dataframe(
    ...
    if df_vocab_game.empty:
        ...
        if df_vocab_game.empty:
            ...
            hint_masked = " ".join(["_" if c.isalpha() else c for c in word_str])
            
            with st.container(border=True):
                st.markdown(f"### ❌ 累積答錯題數：`{st.session_state.game_errors} 次` &nbsp;|&nbsp; 🏷️ {target.get('unit_tag', '')}")
                st.markdown(f"**📌 中文釋義：** {target['definition']}")
                st.markdown(f"**📖 基礎例句：** {clean_sentence(target['basic_sentence'])}")
