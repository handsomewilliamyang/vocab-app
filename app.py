...
if user_api_key:
    ...
    st.sidebar.warning("⚠️ 請務必輸入 API Key 以支援 AI 自動查字典功能")
...
def get_word_record_data_via_ai(word, level="國中部"):
    ...
    # 若有設定 API Key，使用 Gemini AI 自動查字典
    if HAS_GEMINI and st.session_state.get("gemini_api_key"):
        try:
            ...
            return {
                ...
                "part_of_speech": simple_s2t_convert(data.get("part_of_speech", "n. / v.")),
                "definition": simple_s2t_convert(data.get("definition", f"{w_clean} (請補充)")),
    ...
    return {
        ...
        "basic_sentence": f"Please learn how to use '{w_clean}' in sentences.",
        "advanced_sentence": "",
        "collocations": f"common {w_clean}"
    }
            ...
            if single_word:
                with st.spinner("🤖 AI 正在查閱字典並生成例句中..."):
                    ...
                    if upsert_word_to_sheet(data, current_unit_tag, active_worksheet):
                        st.success(f"🎉 成功新增單字：{single_word} ({data['definition']})")
                        time.sleep(0.5)
                        st.rerun()
                    else:
                        st.error("❌ 寫入失敗")
